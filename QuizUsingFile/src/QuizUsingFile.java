...
public class QuizUsingFile
{
    public static void main(String[] args)
    {
        ...
        String entry;
        boolean isGood;
        Path highScoreFile = Paths.get("HighScore.txt");
        int highScore = 0;
        try
        {
            highScoreFile.toFile().createNewFile();
            InputStream input = new BufferedInputStream(Files.newInputStream(highScoreFile, CREATE));
            OutputStream output = new BufferedOutputStream(Files.newOutputStream(highScoreFile, CREATE));
            ...
            else if(Integer.parseInt(scoreLine) < correct)
            {
                highScore = correct;
                writer.write(String.valueOf(highScore));
            }
            else
                highScore = Integer.parseInt(scoreLine);
            JOptionPane.showMessageDialog(null, "Your high score is " + highScore);
            writer.close();
            reader.close();
        }
        catch(Exception e)
