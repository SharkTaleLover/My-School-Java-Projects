...
public class TicTacToe2
{
    ...
    private static boolean gameIsOver(char userOrComp)
    {
        ...
        {
            return true;
        } else if (spots[0] == userOrComp && spots[4] == userOrComp && spots[8] == userOrComp)
        {
            return true;
        } else
        {
    ...
    private static boolean isSquareWinnable(int square)
    {
        spots[square] = 'O';

        if(gameIsOver('O'))
        {
            spots[square] = (char)(square + 1 + 48);
            return true;
        }
        else
        {
    ...
    private static boolean isSquareEmpty(int square)
    {
        if(spots[square] == 'X' || spots[square] == 'O')
        {
            return false;
        }
        else
    ...
    private static int getComputerChoice()
    {
        ...
        int twoInARowCheck = twoInARow();

        if(twoInARowCheck != -1)
        {
            return twoInARowCheck;
        }
        do
        {
